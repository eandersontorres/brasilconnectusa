...
function PrizeEditor({ group, onClose, onSaved, setToast, adminEmail }) {
  ...
  async function handleSave() {
    ...
    try {
      ...
      const data = await res.json()
      if (!res.ok) throw new Error(data.error)
      setToast({ msg: 'Premiação salva! Todos do grupo já podem ver.', type: 'success' })
      onSaved(data.prize)
      onClose()
    } catch (e) {
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ fontSize: 12, color: '#6b7280', marginBottom: 16 }}>
          Visível para todos os membros do grupo
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="Ex: Quem fizer mais pontos ganha uma pizza família e uma cerveja. Em caso de empate, vale o número de placares exatos."
              style={{
                width: '100%', padding: '12px 14px', borderRadius: 10,
                border: '1.5px solid #e5e7eb', fontSize: 14, outline: 'none',
                background: '#fff', boxSizing: 'border-box', fontFamily: 'inherit', resize: 'vertical',
              }}
            />
          </div>
          ...
          <div style={{ display: 'flex', gap: 8, marginTop: 8 }}>
            ...
            <Btn onClick={handleSave} disabled={saving} type="button" style={{ flex: 2 }}>
...
// ════════════════════════════════════════════════════════════════════════════
//   View: Group Dashboard (com premiação + admin badge)
// ════════════════════════════════════════════════════════════════════════════
function GroupDashboard({ group, member, onPredict, onStandings, onLeave, setToast, refreshGroup, deadline }) {
  ...
  const handleShare = async () => {
    ...
    if (r === 'copied') setToast({ msg: 'Link copiado! Cola no WhatsApp 🚀', type: 'success' })
  }
  ...
  return (
    <div style={{ padding: '0 0 16px' }}>
      {editPrize && (
        <PrizeEditor
          group={group}
          onClose={() => setEditPrize(false)}
          onSaved={() => refreshGroup()}
          setToast={setToast}
          adminEmail={adminEmailLocal}
        />
      )}
      ...
      <div style={{
        ...
        borderRadius: 14, padding: '18px 16px', marginBottom: 14, color: '#fff',
      }}>
        <div style={{ fontSize: 11, opacity: 0.85, marginBottom: 4, letterSpacing: 1, textTransform: 'uppercase' }}>
          ⚽ Seu bolão
        </div>
      ...
      {deadlineDate && (
        <div style={{
          ...
          color: expired ? '#991B1B' : '#92400E', marginBottom: 12,
          display: 'flex', alignItems: 'center', gap: 8,
        }}>
          {expired ? '🔒' : '⏰'}
          <div style={{ flex: 1 }}>
      ...
      {/* Card Premiação */}
      <div style={{
        background: hasPrize ? '#FFFBEB' : '#F9FAFB',
        border: '1.5px solid ' + (hasPrize ? GOLD : '#e5e7eb'),
        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10 }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            {hasPrize ? (
              <>
                {group.prize_title && (
                  <div style={{ fontSize: 16, fontWeight: 800, color: '#78350F', marginBottom: 4 }}>
                    {group.prize_title}
                  </div>
                )}
                {group.prize_description && (
                  <div style={{ fontSize: 13, color: '#78350F', lineHeight: 1.5, marginBottom: 6 }}>
                    {group.prize_description}
                  </div>
                )}
        ...
        {isAdmin && (
          <button onClick={() => setEditPrize(true)} style={{
            marginTop: 10, width: '100%', background: 'transparent', border: '1px solid ' + GOLD,
            color: '#8C6D3D', borderRadius: 8, padding: '8px',
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginBottom: 16 }}>
        <button onClick={onPredict} style={{
          ...
          <div style={{ fontSize: 12, color: '#6b7280', marginTop: 2 }}>
            {expired ? '🔒 Palpites travados' : 'Preveja os placares de todas as partidas'}
          </div>
        </button>
          ...
          <div style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: 10, overflow: 'hidden' }}>
            {members.map((m, i) => (
              <div key={m.id} style={{
                ...
                <span style={{ flex: 1, fontSize: 14, fontWeight: m.id === member.id ? 700 : 400 }}>
                  {m.nickname}
                  {m.id === member.id && <span style={{ color: GREEN, fontSize: 11 }}> (você)</span>}
                </span>
                {m.state && (
                  <span style={{ fontSize: 10, fontWeight: 700, color: '#6b7280', background: '#f3f4f6', padding: '2px 6px', borderRadius: 4 }}>
                    {m.state}
                  </span>
                )}
              </div>
  ...
  return (
    <div style={{ padding: '0 0 16px' }}>
      ...
      <div style={{ fontSize: 12, color: '#6b7280', marginBottom: 12 }}>
        Salvo automaticamente · palpites em verde estão registrados
      </div>

      {expired && (
        <div style={{
          background: '#FEE2E2', border: '1px solid #FCA5A5', borderRadius: 10,
          padding: '12px 14px', fontSize: 13, color: '#991B1B', marginBottom: 14,
                  ...
                  <div style={{ display: 'flex', gap: 4, alignItems: 'center', flexShrink: 0 }}>
                    <input
                      ...
                      onBlur={e => {
                        ...
                        if (e.target.value !== '' && a !== undefined && !locked) savePrediction(match.id, e.target.value, a)
                      }}
                      style={{
                    ...
                    <input
                      ...
                      onBlur={e => {
                        ...
                        if (e.target.value !== '' && h !== undefined && !locked) savePrediction(match.id, h, e.target.value)
                      }}
                      style={{
                        width: 44, height: 44, textAlign: 'center', fontSize: 20,
                        fontWeight: 700, borderRadius: 8,
...
function StandingsView({ group, member, onBack, setToast }) {
  ...
  useEffect(() => {
    if (tab === 'group' && !groupStand && group) {
      fetch('/api/bolao?action=standings&group_id=' + group.id)
        .then(r => r.json())
        .then(d => setGroupStand(d.standings || []))
        .catch(() => setGroupStand([]))
    }
    if (tab === 'state' && !stateStand && member.state) {
      fetch('/api/bolao?action=standings-state&state=' + member.state)
        .then(r => r.json())
        .then(d => setStateStand(d.standings || []))
        .catch(() => setStateStand([]))
    }
    if (tab === 'global' && !globalStand) {
      fetch('/api/bolao?action=standings-global')
        .then(r => r.json())
        .then(d => setGlobalStand(d.standings || []))
        .catch(() => setGlobalStand([]))
    }
  ...
  const handleShare = async () => {
    const r = await shareNative({
      ...
      text: '🏆 Confere meu ranking no Bolão Copa 2026! Bora competir? https://brasilconnectusa.com',
    })
    if (r === 'copied') setToast({ msg: 'Link copiado pra compartilhar!', type: 'success' })
  }
          ...
          {standings.map((s, i) => {
            ...
            return (
              <div key={s.member_id} style={{
                ...
                <div style={{ fontSize: 20, width: 30, textAlign: 'center', flexShrink: 0, fontWeight: 700, color: i < 3 ? undefined : '#9ca3af' }}>
                  {medals[i] || (i + 1) + 'º'}
                </div>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontSize: 14, fontWeight: 700, display: 'flex', alignItems: 'center', gap: 6 }}>
                    ...
                    {tab !== 'group' && s.state && (
                      <span style={{ fontSize: 9, fontWeight: 700, color: '#6b7280', background: '#f3f4f6', padding: '1px 5px', borderRadius: 3 }}>
                        {s.state}
                      </span>
                    )}
                  </div>
...
export default function BolaoScreen() {
  ...
  // Carrega config (deadlines, datas)
  useEffect(() => {
    fetch('/api/bolao?action=config')
      .then(r => r.json())
    ...
    fetch('/api/bolao?action=group&code=' + code)
      ...
      .then(d => {
        if (d.group) {
          ...
          if (me) {
            // member da listagem não traz email/state — usa do localStorage
            const fullMe = {
              ...me,
              state: localStorage.getItem('bolao_member_state') || me.state,
            }
            setMember(fullMe)
            setView('group')
          }
  ...
  function handleJoined(g, m) {
    ...
    localStorage.setItem('bolao_member_id', m.id)
    localStorage.setItem('bolao_member_state', m.state || '')
    // Não-admin: limpa qualquer admin_email residual
    localStorage.removeItem('bolao_admin_email')
    setGroup(g); setMember(m); setView('group')
  }

  function handleLeave() {
    ['bolao_member_id','bolao_group_id','bolao_join_code','bolao_admin_email','bolao_member_state']
