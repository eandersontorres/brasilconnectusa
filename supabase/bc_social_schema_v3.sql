...
-- e recria do zero. Use sempre que quiser começar limpo.
-- ═════════════════════════════════════════════════════════════════════════════

-- ── 0. DROP TUDO em ordem reversa de dependência ──────────────────────────
DROP TABLE IF EXISTS bc_reports          CASCADE;
DROP TABLE IF EXISTS bc_event_rsvps      CASCADE;
DROP TABLE IF EXISTS bc_votes            CASCADE;
DROP TABLE IF EXISTS bc_comments         CASCADE;
DROP TABLE IF EXISTS bc_posts            CASCADE;
DROP TABLE IF EXISTS bc_community_members CASCADE;
DROP TABLE IF EXISTS bc_communities      CASCADE;

-- Drop triggers/functions órfãs
...
CREATE INDEX idx_comm_type  ON bc_communities(type);
CREATE INDEX idx_comm_state ON bc_communities(geo_state);

-- ── 2. COMMUNITY MEMBERS ───────────────────────────────────────────────────
CREATE TABLE bc_community_members (
  id            UUID DEFAULT gen_random_uuid() PRIMARY KEY,
...
CREATE INDEX idx_cm_user      ON bc_community_members(user_id);
CREATE INDEX idx_cm_community ON bc_community_members(community_id);
...
CREATE INDEX idx_posts_community ON bc_posts(community_id, created_at DESC);
CREATE INDEX idx_posts_author    ON bc_posts(author_id);
CREATE INDEX idx_posts_type      ON bc_posts(type);
...
CREATE INDEX idx_comments_post   ON bc_comments(post_id, created_at);
CREATE INDEX idx_comments_parent ON bc_comments(parent_id);
CREATE INDEX idx_comments_author ON bc_comments(author_id);

-- ── 5. VOTES ───────────────────────────────────────────────────────────────
CREATE TABLE bc_votes (
  id              UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  user_id         UUID NOT NULL,
...
CREATE INDEX idx_votes_target ON bc_votes(target_type, target_id);

-- ── 6. EVENT RSVPs ─────────────────────────────────────────────────────────
CREATE TABLE bc_event_rsvps (
  id          UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  post_id     UUID NOT NULL REFERENCES bc_posts(id) ON DELETE CASCADE,
  user_id     UUID NOT NULL,
...
CREATE INDEX idx_rsvp_post ON bc_event_rsvps(post_id);
...
-- ── 10. SEEDS — Comunidades por ESTADO ────────────────────────────────────
...
-- ── 11. SEEDS — Comunidades por CIDADE ────────────────────────────────────
...
-- ── 12. SEEDS — Comunidades de INTERESSE ──────────────────────────────────
INSERT INTO bc_communities (slug, name, type, description, is_official) VALUES
...
-- ── 13. VERIFICAÇÃO ───────────────────────────────────────────────────────
SELECT
  'Schema social v3 aplicado com sucesso (RESET completo)' AS status,
